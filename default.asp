...
%></td>
    ...
    <td width="587" colspan="2" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="3">
      ...
      <tr>
		...
		<!-- InstanceBeginEditable name="Conteudo" --><p class="titulo_areas">Noticias</p>
		<table width="100%" border="0" cellspacing="3" cellpadding="5">
		<%
			
			db "leitura",rs,sql,"select top 1 * from noticiashow where destaque>1",conn
			while not rs.eof
		%>
		
		  <tr><td class="table_not_dest">
		<div>
			<%
				if rs("foto") <> "" then
			%>
		<img src="fotos/<%=rs("foto")%>" alt="" name="imgnot" width="65" id="imgnot" />
			...
			%>
		<span class="texto_1"><a href="shownoticia.asp?id=<%=rs("cod")%>"><%=rs("titulo")%></a></span><br />
		<span class="texto_2"><%=rs("comentario")%></span></div>
		</td> </tr>
		 <%
				rs.movenext
			wend
		...
		<%
			...
			db "leitura",rs,sql,"select top 3 * from noticiashow where destaque=1",conn
			while not rs.eof
		%>
		...
		<%
			...
			db "leitura",rs,sql,"select top 5 * from noticiashow where destaque=0",conn
			while not rs.eof
		%>
		...
		<span class="texto_1"><a href="shownoticia.asp?id=<%=rs("cod")%>"><%=rs("titulo")%></a></span><br />
		 ...
		 %> 		 
		  ...
		  <!-- Noticias -->
		  <br />
		  <span class="titulo_areas">&Aacute;lbum </span><br />
		  <table width="90%" border="0" align="center" cellpadding="2" cellspacing="3">
            <tr>
			<%
				db "leitura",rs,sql,"select top 4 * from AlbumLista",conn
				while not rs.eof
			%>
              <td width="86"><a href="album.asp?id=<%=rs("cod")%>"><img src="fotos/<%=trim(rs("foto"))%>" width="86" height="63" hspace="2" vspace="2" border="0" class="table_submaster" /></a></td>
			<%
					rs.movenext
				wend
			%>
