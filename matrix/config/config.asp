...
'Usa-se: emails "de@site.com.br","para@site.com.br","assunto","mensagem","html ou texto"
	Dim email_r, emailsms, sch, cdoConfig, cdoMessage, tpSend
...
Function GridASP(Gsql,Gbase,rp)
	...
	Dim iRecordsShown 
	Dim Grid_semr
	Dim Grid_pgi,Grid_pgf, GFinal, GridMacro, Gfield
	Dim Gridvar, GridX, Glink, Glinkx, GlinkVar, Gridvars, GridPosi
...
GridASP ="<table border=""0"" cellpadding=""3"" cellspacing=""2"" class=""borda_table"" align="""& GridPoss &""" width=" & Glink & ">"&_
 "<tr>"&_
	 ...
	 for GridX = LBound(GridName) to UBound(GridName)
	   ...
	   if Gridvar(1) <> 0 then
		   GridASP = GridASP & "<td height=24 width="""&Gridvar(1)&""" align=""center"" "
			...
			GridASP = GridASP & "<td height=24  align=""center"" "
		end if
	   
	   if GridX = UBound(GridName) then
      		GridASP = GridASP & "class=""titulo_td_fim"""
				if left(Gridvars(0),1)<>"*" and left(Gridvars(0),1)<>"$"  then
		...
		'Listar Dados das Colunas ========================================================
				for GridX = LBound(GridData) to UBound(GridData)
					...
					if Gridvar(2)<>"0" or Gridvar(2)<> null then	
						...
						Glink = replace(Glink,"[mod]", request.form("m"))
					...
					if cstr(left(Gridvar(0),1)) = "*" then
						...
						'verifica a existencia de outros valores
							...
							GridMacro = replace(GridMacro,"[mod]", request.form("m"))
							for Glinkx = LBound(GridData) to UBound(GridData)
 ...
 GridASP = GridASP &   "   <table width=""100%"" border=""0"" cellpadding=""0"" cellspacing=""0"" class=""texto"">"&_
       " <tr class=""fundo_base"">"&_
		...
		"<input name=""Button"" type=""button"" class=""button"""
		 ...
		 GridASP = GridASP & " value=""&lt;&nbsp;Anterior "" />"
 ...
 GridASP = GridASP &  "		 </td>          <td width=""33%"" align=""center"" valign=""middle"" class=""texto_11"">"
		  ...
		  If cint(iPageCurrent) < cint(iPageCount) Then
			...
			GridASP = GridASP &  " onclick=""Gpages("& cint(iPageCurrent) + 1 &");"""
		 	 ...
		 	 GridASP = GridASP &  " value=""Pr&oacute;ximo&nbsp;&gt;"" />"
			 
		 GridASP = GridASP &    " </td>        </tr> "
		 '//-------------------------------------------
		 '//Filtros 
		 
			 GridASP = GridASP &	"	  <tr class=""hide"" id=""filtrogrid"">" &_
   " <td colspan=""3"" align=""left"" class=""areaspecial""><span class=""texto_11"">Filtro</span><br />" &_
  "    <input name=""filtroaspgrid"" type=""text"" class=""form_texto"" id=""filtroaspgrid"" />" &_
  "    &nbsp;" &_
  "    <select name=""filtrotablegrid"" class=""form_texto"" id=""filtrotablegrid"">" 
  	for GridX = LBound(GridData) to UBound(GridData)
		
		Gridvar = split(GridData(GridX),";")
		if left(Gridvar(0),1) <> "*" and left(Gridvar(0),1) <> "$" then
			Gridvars = split(GridName(GridX),";")
			GridASP = GridASP & "<option value='"&Gridvar(0)&"'>"&Gridvars(0)&"</option>"
		end if
	next
  GridASP = GridASP &  "    </select>" &_
    "  &nbsp;" &_
 "     <input name=""Button"" type=""button"" class=""button"" value=""Filtrar"" onclick=""if(document.getElementById('filtroaspgrid').value!=''){AspGridAct('Filtro')}"" /></td>" &_
  "  </tr>" &_
 " <tr class=""hide"" id=""ordernargrid"">" &_
  "  <td colspan=""3"" align=""left"" class=""areaspecial""><span class=""texto_11"">Ordernar por...</span><br />" &_
    "  <select name=""tablegrido"" class=""form_texto"" id=""tablegrido"">" 
  ...
  GridASP = GridASP & "    </select>" &_
  "    &nbsp;" &_
  "    <select name=""ordertablegrid"" class=""form_texto"" id=""ordertablegrid"">" &_
   "     <option value=""asc"" selected=""selected"">Crescente</option>" &_
     "   <option value=""desc"">Decrescente</option>" &_
   "   </select>      " &_
 "     &nbsp;" &_
 "     <input name=""Button"" type=""button"" class=""button"" value=""Ordenar"" onclick=""AspGridAct('Order')"" /></td>" &_
"  </tr>" &_
 " <tr>" &_
 "   <td colspan=""3"" align=""left"" class=""fundo_base_gray"">&nbsp;" &_
     " <input name=""Buttons"" type=""button"" class=""button"" value=""Filtro"" onclick=""showitem('filtrogrid','ordernargrid');""/>" &_
      "  <input name=""Buttons"" type=""button"" class=""button"" value=""Ordenar"" onclick=""showitem('ordernargrid','filtrogrid');""/>" &_
	   "  <input name=""Buttons"" type=""button"" class=""button"" value=""Listar Todos"" onclick=""AspGridAct('');""/></td>" &_
 " </tr>"
		 
		 '//-------------------------------------------
		 
		GridASP = GridASP &    "  </table> "
		end if
	...
	'//----------------------------------------------->
		Dim GridX, GridCount, GridURLS, objBCap
			function GridForm(key,r)
				GridCount = 0
				GridForm = "<form>"&br
			GridForm = GridForm & "<script>"&br&"/*-------------------------------"&br&"AjaxGrid v1.8 "&br&"CodeBase: ASP"&br&"Desenvolvedor: Paulo Corcino"&br&br&"-------------------------- */"&br&"function showitem(gobj,gobj2){"&br

			GridForm = GridForm & "if(document.getElementById(gobj).className == 'hide')"&br
			GridForm = GridForm & "{"&br
			GridForm = GridForm & "document.getElementById(gobj).className = 'show';"&br
			
			GridForm = GridForm & "document.getElementById(gobj2).className = 'hide';"&br
			GridForm = GridForm & "}else{"&br
			GridForm = GridForm & "document.getElementById(gobj).className = 'hide';"&br
			GridForm = GridForm & "document.getElementById(gobj2).className = 'hide';"&br
			GridForm = GridForm & "}"&br

			GridForm = GridForm & "}; var AspGridAction = ''; function AspGridAct(tp){AspGridAction=tp;"&br
			GridForm = GridForm & "if(tp=='Order'){ "&br
			GridForm = GridForm & "	document.getElementById('GridOrder').value = document.getElementById('tablegrido').value;"&br
			GridForm = GridForm & "	document.getElementById('GridOrderF').value = document.getElementById('ordertablegrid').value;"&br
			GridForm = GridForm & "	document.getElementById('GridFilter').value = '';"&br
			GridForm = GridForm & "	document.getElementById('GridFilterF').value = '';"&br

			GridForm = GridForm & "}"&br
			GridForm = GridForm & "else"&br
			GridForm = GridForm & "{"&br
			GridForm = GridForm & "	document.getElementById('GridFilter').value = document.getElementById('filtroaspgrid').value;"&br
			GridForm = GridForm & "	document.getElementById('GridFilterF').value = document.getElementById('filtrotablegrid').value;"&br
			GridForm = GridForm & "	document.getElementById('GridOrder').value = '';"&br
			GridForm = GridForm & "	document.getElementById('GridOrderF').value = '';"&br
			GridForm = GridForm & "}"
			...
			GridForm = GridForm & "} </script>"&br
				...
				GridForm = GridForm & "<input name='GridPoss' id='GridPoss' type='hidden' value='"&Encrypt(GridPoss,key)&"' />"&br
				...
				GridForm = GridForm & "<input name='GridOrder' id='GridOrder' type='hidden' value='' />"&br
				GridForm = GridForm & "<input name='GridOrderF' id='GridOrderF' type='hidden' value='' />"&br
				GridForm = GridForm & "<input name='GridFilter' id='GridFilter' type='hidden' value='' />"&br
				GridForm = GridForm & "<input name='GridFilterF' id='GridFilterF' type='hidden' value='' />"&br
			...
			GridForm = GridForm & "		//document.getElementById('textss').value = unescape(http.responseText.replace(/\+/g,' '));"&br
			...
			'GridForm = GridForm & "				inc = 'M="&request.QueryString("m")&"&ASPID="&aspid&"&GPAGE='+n+'&GRIDR='+document.getElementById('GridR').getAttribute('value')+'&GRIDFilterF=' + document.getElementById('GridFilterF').getAttribute('value') + '&GRIDFilter=' + document.getElementById('GridFilter').getAttribute('value') + '&GRIDORDERF=' + document.getElementById('GridOrderF').getAttribute('value') + '&GRIDORDER=' + document.getElementById('GridOrder').getAttribute('value') + '&GRIDN=' + document.getElementById('GridN').getAttribute('value') + '&GridName_' + i + '=' + document.getElementById('GridName_'+i).getAttribute('value') + '&GridData_' + i + '=' + document.getElementById('GridData_'+i).getAttribute('value');"&br
			GridForm = GridForm & "				inc = 'M="&request.QueryString("m")&"&ASPID="&aspid&"&GPAGE='+n+'&GRIDR='+document.getElementById('GridR').getAttribute('value');"&br
				
			GridForm = GridForm & "		if(AspGridAction=='Filtro'){"&br
			GridForm = GridForm & "	inc = inc + '&GRIDFilterF=' + document.getElementById('GridFilterF').getAttribute('value') + '&GRIDFilter=' + document.getElementById('GridFilter').getAttribute('value');"&br
			GridForm = GridForm & "	}else{"&br
			GridForm = GridForm & "		if(AspGridAction=='Order')"&br
			GridForm = GridForm & "		{"&br
			GridForm = GridForm & "			inc = inc + '&GRIDORDERF=' + document.getElementById('GridOrderF').getAttribute('value') + '&GRIDORDER=' + document.getElementById('GridOrder').getAttribute('value');"&br
			GridForm = GridForm & "		}"&br
			GridForm = GridForm & "	};"&br
				
			GridForm = GridForm & "	inc = inc + '&GRIDN=' + document.getElementById('GridN').getAttribute('value') + '&GridName_' + i + '=' + document.getElementById('GridName_'+i).getAttribute('value') + '&GridData_' + i + '=' + document.getElementById('GridData_'+i).getAttribute('value');"&br
			GridForm = GridForm & "			}else{"&br
 ...
 dim txtTmp, tTmp, tlTmp, cabTmp, XMLHttp, tTenpV, nCidades, nCidadesPI, nCidadesPF
 dim tmpA,tmpB,tmpC,tmpAm,tmpBm, tmpCm, tempSplit, tempSplitTotal, tmpX, tmpPos
		...
		If fso.FileExists(server.MapPath(tempo_root&fDate(DateAdd("D", 1, Date),"ddmmyyyy")&".js")) = false then
			...
			'	response.write tTmp
				if tempoEstado = "br" then
					...
					'&tpmin2=&tpmin3=&tpmax1=&tpmax2=&tpmax3=&tpatual=&iconatual=nb
					tTenpV = left(tTmp,instr(tTmp,"&total=")-1)
					tTmp = replace(tTmp,tTenpV,"")
					tTenpV = replace(tTenpV,"'","")
					nCidadesPI = instr(tTmp,"&total=")+7
					nCidadesPF = instr(tTmp,"','&cidade=")-1
					nCidades = mid(tTmp,nCidadesPI,instr(tTmp,"','&cidade=")-nCidadesPF)
					tempSplit = split(tTenpV,",")
					tempSplitTotal = ubound(tempSplit) - (ubound(tempSplit)-(nCidades*6))
					tmpPos = 0
					'// Variaveis Importantes  --------------------
					'tTenpV - Temperatura mesclada
					'nCidades - numero de cidades
					
					for tmpX = 1 to nCidades
						tmpA = tmpA&"'"&tempSplit(tmpPos)&"',"
						tmpPos = tmpPos + 1
					next
						tmpA = left(tmpA,len(tmpA)-2)
					
					for tmpX = 1 to nCidades
						tmpB = tmpB&"'"&tempSplit(tmpPos)&"',"
						tmpPos = tmpPos + 1
					next
						tmpB = left(tmpB,len(tmpB)-2)
					
					for tmpX = 1 to nCidades
						tmpC = tmpC&"'"&tempSplit(tmpPos)&"',"
						tmpPos = tmpPos + 1
					next
						tmpC = left(tmpC,len(tmpC)-2)
					
					for tmpX = 1 to nCidades
						tmpAm = tmpAm&"'"&tempSplit(tmpPos)&"',"
						tmpPos = tmpPos + 1
					next
						tmpAm = left(tmpAm,len(tmpAm)-2)
					
					for tmpX = 1 to nCidades
						tmpBm = tmpBm&"'"&tempSplit(tmpPos)&"',"
						tmpPos = tmpPos + 1
					next
						tmpBm = left(tmpBm,len(tmpBm)-2)
					
					for tmpX = 1 to nCidades
						tmpCm = tmpCm&"'"&tempSplit(tmpPos)&"',"
						tmpPos = tmpPos + 1
					next
						tmpCm = left(tmpCm,len(tmpCm)-2)
					
					'response.write tempSplitTotal
					tTmp = replace(tTmp,"&total=","var Total = new Array('")
					tTmp = replace(tTmp,"','&cidade=","');"&vbcrlf&"var Cidade = new Array('")
					tTmp = replace(tTmp,"','&clima1=","');"&vbcrlf&"var Clima1 = new Array('")
					tTmp = replace(tTmp,"','&clima2=","');"&vbcrlf&"var Clima2 = new Array('")
					tTmp = replace(tTmp,"','&clima3=","');"&vbcrlf&"var Clima3 = new Array('")
					tTmp = replace(tTmp,"','&tpmin1=","');"&vbcrlf&"var Tpmin1 = new Array("&tmpA)
					tTmp = replace(tTmp,"&tpmin2=","');"&vbcrlf&"var Tpmin2 = new Array("&tmpB)
					tTmp = replace(tTmp,"&tpmin3=","');"&vbcrlf&"var Tpmin3 = new Array("&tmpC)
					tTmp = replace(tTmp,"&tpmax1=","');"&vbcrlf&"var Tpmax1 = new Array("&tmpAm)
					tTmp = replace(tTmp,"&tpmax2=","');"&vbcrlf&"var Tpmax2 = new Array("&tmpBm)
					tTmp = replace(tTmp,"&tpmax3=","');"&vbcrlf&"var Tpmax3 = new Array("&tmpCm)
					tTmp = replace(tTmp,"&tpatual=","');"&vbcrlf&"var Tpatual= new Array('")
					tTmp = replace(tTmp,"&iconatual=","');"&vbcrlf&"var Iconatual= new Array('")
					tTmp = replace(tTmp,"','&link=","');"&vbcrlf&"var Link= new Array('")
					tlTmp= len(tTmp) - 2
				...
				'response.write cabTmp&vbcrlf&tTmp
...
response.write "<form name='enqForm'>" &_
	...
	if not enqtrs.eof and not enqtrs.bof then
...
dim imgfEnqV, imgfEnqR

function feEnquete(cod)

startjs() '//Inicia o script js
response.write "<script language=""JavaScript"" type=""text/javascript"" src="""&site&"/matrix/enquetefoto/nEnquete.js""></script>"
response.write "<form name='enqfForm'>" &_
				"<div id='feEnquete'></div>" &_
				"</form>"

	dim isresult, enqtrs, enqtsql
	isresult = 0
	
	db "leitura",enqtrs,enqtsql,"select top 1 * from enquetefotoshow where id_a="&cod,conn
	if not enqtrs.eof and not enqtrs.bof then
		if request.Cookies("feEnquete"&enqtrs("id")) = "votado" then
			isresult = 1
		end if
		
		response.write "<script>"
		if isresult = 0 then
			response.write "AjaxLoad("""&site&"/matrix/enquetefoto/inc_enquete.asp?op=&ida="&cod&"&html="&request("html")&"&imgv="&imgfEnqV&"&imgr="&imgfEnqR&""",null,""feEnquete"")"
		else
			response.write "AjaxLoad("""&site&"/matrix/enquetefoto/inc_enquete.asp?op=result&ida="&cod&"&html="&request("html")&"&imgv="&imgfEnqV&"&imgr="&imgfEnqR&""",null,""feEnquete"")"
		end if
...
'///---- Sistema de Banners ------------------------------------------------
dim BanValid
function SysBanner(banArea)
	startjs()
	BanValid = false
	dim banrs, bansql, wbans, hbans, tbans
	'banArea = "bannertopo"
	db "leitura",banrs,bansql,"select * from tb_area_sysban where chave_area_sysban='"&banArea&"'",conn
	if not banrs.eof and not banrs.bof then
		wbans = banrs("w_area_sysban")
		hbans = banrs("h_area_sysban")
		tbans = banrs("t_area_sysban") * 1000
			db "leitura",banrs,bansql,"select top 1 cod_ban_sysban from BannerShow where cod_area_sysban='"&banrs("cod_area_sysban")&"'",conn
			if not banrs.eof and not banrs.bof then
				BanValid = true
			else
				BanValid = false
	...
	if BanValid then
		response.write "<div id="""&banArea&""" style=""width:"&wbans&"px; min-width:"&wbans&"px""></div>"
		response.write "<script>"
			response.write "function "&banArea&"timer(){"
			response.write "AjaxLoad('"&site&"/matrix/app_banner/show_banner.asp?key="&banArea&"',null,'"&banArea&"');"
			response.write banArea&"tim = setTimeout('"&banArea&"timer()',"&tbans&");}"
			response.write banArea&"timer();"
		response.write "</script>"
	end if
end function
...
'/----------------------------------------------------------------------------
'//-----------------------------------
'// Função pra verificar pastas Fundamentais
'//-----------------------------------
	CadFile = roots&"/startsys.txt"
	...
	If fso.FileExists(server.MapPath(CadFile)) = false then
		...
		criatxt CadFile, 1, "Criado pastas de sistemas - "& now()
