 ...
 %>
     <tr>
      <td id="menutab">
		...
		redim AspMenuOp(2) 'Define o numero de itens
		'//campos ---------------------------------
		AspMenuOp(0) = "Voltar|1|WindowOpen('default.asp?op=list_noticia&periodo="&formus("periodo",3)&"&id_s="&formus("id_s",3)&"&aspid=' + AspId)"
		AspMenuOp(1) = "Salvar|3|miniMenu_dest()"
		'//----------------------------------------
		AspMenu() 'gera menu
		'// Gera Menus ---------------------------- >
		%>
    ...
    <tr>
      <td id="conteudo_page">
	    <p><span class="texto_size_10">Qual o tipo de destaque destas noticias selecionadas</span><br />
		<input name="sCod_vale" type="hidden" value="<%=formus("sCod_vale",0)%>" />
		<input name="periodo" type="hidden" value="<%=formus("periodo",3)%>" />
		<input name="id_s" type="hidden" value="<%=formus("id_s",3)%>" />
	      <select name="sDst">
	        <option value="0">0 - Sem Destaque</option>
	        <option value="1">1 - Destaque Principal</option>
	        <option value="2">2 - SubDestaque</option>
	        <option value="3">3 - SubDestaque</option>
	        <option value="4">4 - SubDestaque</option>
          </select>
</p>
      </td>
    </tr>	
  	...
  	case "dest_op"
		...
		ids = split(formus("sCod_vale",0),",")
			...
			for x = lbound(ids) to ubound(ids)
				...
				'else
					db "normal",,sql,"update not_noticia set dst="&formus("sDst",0)&" where id="&ids(x),conn
				'end if
			
			next
  		voltar request.ServerVariables("SCRIPT_NAME")&"?aspid="&aspid&"&op=list_noticia&periodo="&formus("periodo",0)&"&id_s="&formus("id_s",0),"","_self"
