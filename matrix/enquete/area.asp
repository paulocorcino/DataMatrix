...
<%
		elseif not isempty(request.form("submit")) and trim(formus("area",1))<>"" then
			...
			db "leitura",rs,sql,"select id from enquete_areas where areas='"&formus("area",1)&"'",conn
				if rs.eof and rs.bof then
					db "normal",,sql,"insert into enquete_areas(areas) values ('"&formus("area",1)&"')",conn
	...
	case "alt"
		...
		db "leitura",rs,sql,"select id, areas from enquete_areas where id="&request.QueryString("id"),conn
		if rs.eof and rs.bof then
...
<%
	elseif not isempty(request.Form("submit")) and trim(formus("area",1))<>"" then
		...
		db "leitura",rs,sql,"select id from enquete_areas where areas='"&formus("area",1)&"' and id<>"&formus("id",0),conn
		if rs.bof and rs.eof then
			db "normal",,sql,"update enquete_areas set areas='"&formus("areas",1)&"' where id="&formus("id",0),conn
			voltar request.ServerVariables("SCRIPT_NAME")&"?op=listar","","_self"
		else
	...
	'Tela de Deletar
	case "del"
		db "leitura",rs,sql,"select id from enquete_areas where id="&request.QueryString("id"),conn
		if rs.eof and rs.bof then
			voltar request.ServerVariables("SCRIPT_NAME")&"?op=listar","","_self"
		else
			db "normal",,sql,"delete from enquete_areas where id="&request.QueryString("id"),conn
			voltar request.ServerVariables("SCRIPT_NAME")&"?op=listar","","_self"
		end if
