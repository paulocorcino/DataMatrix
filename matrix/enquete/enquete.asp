	...
	select case request.QueryString("op")
		...
		if request.QueryString("alt")<>"" and request.QueryString("alt")<>0 then
			...
			if request.QueryString("id")<>"" and request.QueryString("id")<>0 then
				...
				db "leitura",rs,sql,"select id, id_a, criado, entrada, saida, pergunta,alt from dbo.enquete_perguntas where id="&request.QueryString("id"),conn
				er(30)
				x = trim(rs("pergunta"))
				...
				db "leitura",rs,sql,"select id, id_a, criado, entrada, saida, pergunta,alt from dbo.enquete_perguntas where criado=0 ",conn
				er(37)
				x = trim(rs("pergunta"))
			...
			db "leitura",rs,sql,"select id, id_a, criado, entrada, saida, pergunta,alt from enquete_perguntas where criado=0",conn
			er(47)
				if rs.eof and rs.bof then
				
					db "normal",,sql,"insert into enquete_perguntas(criado,alt,janelanova,totalvotos,repeteuser,color) values (0,2,0,1,0,0)",conn
					db "leitura",rs,sql,"select id, id_a, entrada, saida, criado,pergunta,alt from enquete_perguntas where criado=0",conn
					er(50)
...
<table width="98%"  border="0" cellspacing="2" cellpadding="0">
  <tr>
    <td class="fundo2">&nbsp;&nbsp;<span class="fonte"><strong>Nova Enquete - Passo 1 / 4 </strong></span></td>
  </tr>
  <tr>
    <td><%=erros%></td>
  </tr>
      ...
      <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="2">
        ...
        <tr>
		  ...
		  <select name="id_a" class="formtextarea" id="id_a">
		  	<%
				db "leitura",rs2,sql,"select * from enquete_areas",conn
				while not rs2.eof
			%>
					<option value="<%=rs2("id")%>" 
					<% 
...
<%
	...
	if request.QueryString("alt")<>"" and request.QueryString("alt")<>0 then
			...
			db "leitura",rs,sql,"SELECT dbo.enquete_perguntas.id, dbo.enquete_perguntas.pergunta,dbo.enquete_perguntas.alt, dbo.enquete_respostas.id AS id_r, dbo.enquete_respostas.resposta, dbo.enquete_respostas.votos FROM dbo.enquete_perguntas INNER JOIN dbo.enquete_respostas ON dbo.enquete_perguntas.id = dbo.enquete_respostas.id_p WHERE (dbo.enquete_perguntas.id = "&request.querystring("id")&request.form("id")&")",conn
			er(122)
			alt=1
	else
	'response.Write "teste"
		db "leitura",rs,sql,"SELECT id, pergunta, entrada, ativo FROM dbo.enquete_perguntas WHERE (id_a = "&formus("id_a",0)&") AND (ativo = 1) AND (criado = 1) and (id <> "&request("id")&") and (entrada >= CONVERT(DATETIME, '"&fdate(formus("entrada",0),"yyyy-mm-dd")&" 00:00:00', 102)) AND (saida <= CONVERT(DATETIME, '"&fdate(formus("saida",0),"yyyy-mm-dd")&" 00:00:00', 102)) OR (entrada <= CONVERT(DATETIME, '"&fdate(formus("saida",0),"yyyy-mm-dd")&" 00:00:00', 102)) AND (saida >= CONVERT(DATETIME, '"&fdate(formus("entrada",0),"yyyy-mm-dd")&" 00:00:00', 102)) and (id <> "&request("id")&") and (id_a = "&formus("id_a",0)&")",conn
		er(127)
		if rs.eof and rs.bof then
			...
			if not trim(request.form("pergunta"))<>"" then
				...
				db "normal",,sql,"update enquete_perguntas set pergunta='"&formus("pergunta",1)&"',entrada='"&x&"',saida='"&fdate(formus("saida",0),"yyyymmdd")&"',ativo=1,alt="&formus("alt",0)&" where id="&formus("id",0),conn
				er(143)
				' x esvasia
				x = empty
				...
				alt = 1
					if rs.eof and rs.bof then
						for x=1 to formus("alt",0)
							...
							db "normal",,sql,"insert into enquete_respostas(id_p,votos) values ("&id_p&",1)",conn
							er(156)
						next
...
<table width="98%"  border="0" cellspacing="2" cellpadding="0">
  <tr>
    <td class="fundo2">&nbsp;&nbsp;<span class="fonte"><strong>Nova Enquete - Passo 2 / 4 </strong></span></td>
  </tr>
  <tr>
    <td><%=erros%></td>
  </tr>
  <tr>
    <td><form name="form" method="post" action="<%=request.ServerVariables("SCRIPT_NAME")%>?op=cad3&aspid=<%=aspid()%>&alt=<%=alt%>">
      <table width="98%"  border="0" align="center" cellpadding="0" cellspacing="2">
        <tr>
          <td class="fonte"><strong>Pergunta</strong></td>
        </tr>
        <tr>
          <td class="fonte"><%=trim(rs("pergunta"))%><input type="hidden" name="id" value="<%=request.QueryString("id")&formus("id",0)%>"></td>
        </tr>
        <tr>
          <td class="fonte">&nbsp;</td>
        </tr>
		...
		%>
				<td width="17%"><input name="alt<%=x%>" type="text" class="fonte" id="alt<%=x%>" size="30" maxlength="50" value=""></td>
		...
		%>
 				<td width="17%"><input name="alt<%=x%>" type="text" class="fonte" id="alt<%=x%>" size="30" maxlength="50" value="<%=trim(rs("resposta"))%>"></td>
...
<%
	...
	'db "leitura",rs,sql,"select pergunta,id,alt from enquete_perguntas where id="&formus("id",0),conn
	db "leitura",rs,sql,"SELECT dbo.enquete_perguntas.id, dbo.enquete_perguntas.pergunta, dbo.enquete_perguntas.alt, dbo.enquete_respostas.resposta, dbo.enquete_perguntas.janelanova, dbo.enquete_perguntas.totalvotos, dbo.enquete_perguntas.repeteuser, dbo.enquete_perguntas.color, dbo.enquete_respostas.votos FROM dbo.enquete_perguntas INNER JOIN dbo.enquete_respostas ON dbo.enquete_perguntas.id = dbo.enquete_respostas.id_p WHERE (dbo.enquete_perguntas.id = "&formus("id",0)&")",conn
	
	db "normal",,sql,"update enquete_respostas set votos=0 where id_p="&formus("id",0),conn
	
	for x=1 to rs("alt")
	
		
		if trim(formus("alt"&x,1))<>"" and trim(formus("votos"&x,0))<>"" then
		
			'certo
			if rs.eof and rs.bof or rs.eof then
				votos = cint(formus("votos"&x,0)) + 1
			'response.Write "3"
					db "normal",,sql,"insert into enquete_respostas(id_p,resposta,votos) values ("&formus("id",0)&",'"&formus("alt"&x,1)&"',"&votos&")",conn
							
			else
				votos = cint(formus("votos"&x,0)) + 1
				'response.Write "2"
					db "normal",,sql,"update enquete_respostas set resposta='"&formus("alt"&x,1)&"',votos="&votos&" where id="&formus("id_r"&x,0),conn
			...
			'errsad
			voltar request.ServerVariables("SCRIPT_NAME")&"?op=cad2&aspdi="&aspid()&"&alt=1&id="&formus("id",0),"Insira as alternativas e coloque um numero de votos igual ou maior que zero.","_self"
			fdb(rs)
			fdb(conn)
			response.End()
	...
	next
	db "normal",,sql,"delete from enquete_respostas where votos=0 and id_p="&formus("id",0),conn
	rs.movefirst
	'db "leitura",rs,sql,"SELECT enquete_perguntas.id, enquete_perguntas.pergunta, enquete_perguntas.alt,respostas.id, respostas.resposta, total_votos.Votos FROM (enquete_perguntas INNER JOIN respostas ON enquete_perguntas.id = respostas.id_p) INNER JOIN total_votos ON respostas.id = total_votos.id WHERE (((enquete_perguntas.id)="&request.querystring("id")&formus("id",0)&"))",conn
	
	
%>
<table width="98%"  border="0" cellspacing="2" cellpadding="0">
  <tr>
    <td class="fundo2">&nbsp;&nbsp;<span class="fonte"><strong>Nova Enquete - Passo 3 / 4 </strong></span></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
...
<%
	...
	db "normal",,sql,"update enquete_perguntas set criado=1, id_a="&session("id_a")&", totalvotos="&formus("totalvoto",0)&", repeteuser="&formus("repeteuser",0)&" where id="&formus("id",0),conn
	
%>
<table width="98%"  border="0" cellspacing="2" cellpadding="0">
  <tr>
    <td class="fundo2">&nbsp;&nbsp;<span class="fonte"><strong>Nova Enquete - Passo 4 / 4 </strong></span></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
...
<%
	...
	case "atv"
		...
		if x = 1 then
			...
			db "normal",rs,sql,"update enquete_perguntas set ativo=0 where id="&request.QueryString("id"),conn
	
		else
	
			db "normal",rs,sql,"update enquete_perguntas set ativo=1 where id="&request.QueryString("id"),conn
	...
	case "del"
		...
		db "normal",,sql,"delete from enquete_perguntas where id="&request.QueryString("id"),conn
		voltar request.ServerVariables("SCRIPT_NAME")&"?id_a="&formus("id_a",3)&"&aspid="&aspid(),"","_self"
	case "result"
...
<table width="100%" border="0" cellspacing="2" cellpadding="0">
	...
	'db "leitura",rs,sql,"SELECT respostas.id, perguntas.pergunta, perguntas.alt, respostas.resposta, Sum(total_votos.SomaDevotos) AS totalvotos, (1/(totalvotos/votos))*100 AS porcenta FROM total_votos INNER JOIN (perguntas INNER JOIN respostas ON perguntas.id = respostas.id_p) ON total_votos.id = perguntas.id GROUP BY respostas.id, perguntas.id, perguntas.pergunta, perguntas.alt, respostas.resposta, respostas.votos, (1/(totalvotos/votos))*100 HAVING (((perguntas.id)="&request.QueryString("id")&")) ORDER BY respostas.id",conn
	db "leitura",rs,sql,"SELECT TOP 100 PERCENT dbo.enquete_perguntas.id, dbo.enquete_respostas.id AS id_r, dbo.enquete_perguntas.pergunta, dbo.enquete_respostas.resposta, dbo.enquete_perguntas.alt, dbo.total_votos.SomaDevotos AS totalvotos, dbo.enquete_respostas.votos FROM dbo.enquete_perguntas INNER JOIN  dbo.enquete_respostas ON dbo.enquete_perguntas.id = dbo.enquete_respostas.id_p INNER JOIN dbo.total_votos ON dbo.enquete_perguntas.id = dbo.total_votos.id WHERE(dbo.enquete_perguntas.id = "&request.QueryString("id")&")",conn
	er(15)
%>
  ...
  <tr>
    <td align="center" valign="top"><form name="form1" method="post" action="">
      <table width="80%" height="100%" border="0" cellpadding="0" cellspacing="1">
		...
		%>
            <tr>
              <td align="center" valign="middle" class="alternativas"><div align="left"><%=rs("resposta")%></div></td>
            </tr>
            <tr>
...
<%
	...
	'db "leitura",rs,sql,"SELECT TOP 100 PERCENT dbo.enquete_perguntas.id, dbo.enquete_perguntas.pergunta, dbo.enquete_perguntas.criado, dbo.enquete_perguntas.ativo, dbo.enquete_perguntas.alt, dbo.enquete_perguntas.entrada, dbo.total_votos.SomaDevotos AS totalvotos FROM dbo.enquete_perguntas INNER JOIN dbo.total_votos ON dbo.enquete_perguntas.id = dbo.total_votos.id WHERE (dbo.enquete_perguntas.criado = 1) ORDER BY dbo.enquete_perguntas.entrada DESC",conn 
	
	prep_deletar()
%>
<table width="98%"  border="0" cellspacing="2" cellpadding="0">
  <tr>
    <td class="fundo2">&nbsp;&nbsp;<span class="fonte"><strong>Listar Enquetes </strong></span></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
  </tr>
  <tr>
	...
	<form name="form1" method="post">
	  ...
	  <%
	  	db "leitura",rs,sql,"select * from enquete_areas",conn
		while not rs.eof
	  %>
	  ...
	  <div align="center">
	    <%
				  Dim GridData(6), GridName(6), GridSQL, GridDate, GridPoss
					...
					GridSQL = "select * from show_enquete where id_a = "&formus("id_a",3)&";"
