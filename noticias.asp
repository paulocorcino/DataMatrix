...
%></td>
    ...
    <td width="587" colspan="2" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="3">
      ...
      <tr>
		...
		<span class="style1">Buscar Noticia		</span>
		<form id="formsn" name="formsn" method="get" action="noticias.asp">
		  <table width="100%" border="0" cellspacing="2" cellpadding="0">
			<tr>
			  <td width="38%"><input name="query" type="text" class="texto_1" size="40" /></td>
			  <td width="62%"><select name="area" class="texto_1" id="area">
			  </select>       <input type="submit" name="Submit" value="buscar" /></td>
			</tr>
		  </table>
		</form>
		<%AddSessionsNoticia("formsn.area")%>
		<span class="titulo_areas">Noticias</span>
		<table width="100%" border="0" cellspacing="3" cellpadding="0">
		<%
			if formus("query",3)<>"" then
								'//-- Busca de noticia
				...
				if query <> "" then
					buscasql = buscasql & " and texto like '%"&query&"%' or comentario like '%"&query&"%' or titulo like '%"&query&"%'"
				else
				...
				db "leitura",rs,sql,"select top 50 * from noticiashow",conn
			end if
			while not rs.eof
		%>
		...
		<span class="texto_2"><%=rs("comentario")%></span></div><br />
		</td> </tr>
		 <%
				rs.movenext
			wend
